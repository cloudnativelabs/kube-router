...
func TestCommonICMPRules(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expected: []ICMPRule{
				{"icmp", "--icmp-type", "echo-request", icmpEchoRequestComment},
				{"icmp", "--icmp-type", icmpTypeDestinationUnreachable, icmpDestinationUnreachableComment},
				{"icmp", "--icmp-type", "time-exceeded", icmpTimeExceededComment},
			},
		},
		{
			name:   "IPv6",
			...
			expected: []ICMPRule{
				{"ipv6-icmp", "--icmpv6-type", "echo-request", icmpEchoRequestComment},
				{"ipv6-icmp", "--icmpv6-type", icmpTypeDestinationUnreachable, icmpDestinationUnreachableComment},
				{"ipv6-icmp", "--icmpv6-type", "time-exceeded", icmpTimeExceededComment},
