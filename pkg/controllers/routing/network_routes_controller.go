...
type NetworkRoutingController struct {
	...
	routerInjector                 pkg.RouteInjector
...
func (nrc *NetworkRoutingController) watchBgpUpdates(ri pkg.RouteInjector) {
	ph := bgp.PathHandler{
		PeerLister:    nrc.bgpServer,
		RouteInjector: ri,
		RouteSyncer:   nrc.routeSyncer,
		TunnelCleaner: nrc.tunneler,
	}
		...
		if table := r.GetTable(); table != nil {
			for _, path := range table.Paths {
				if path.Family.Afi == gobgpapi.Family_AFI_IP ||
					...
					if err := ph.Changed(path); err != nil {
...
func (nrc *NetworkRoutingController) syncNodeIPSets(nodeIPAware pkg.NodeIPAware) error {
	var err error
	start := time.Now()
	defer func() {
		if nrc.MetricsEnabled {
...
func NewNetworkRoutingController(clientset kubernetes.Interface,
	...
	// At some point it would be good to decouple this logic better. Right now LinuxRouter depends on having a RouteSync
	// and RouteSync depends on LinuxRouter. This logic is a bit confusing.

	// Create a new LinuxRouter instance, we purposefully do not set the RouteSyncer here as it is not yet created, plus
	// we don't set it on nrc.routerInjector (which is an interface and does not have access to individual attributes of
	// LinuxRouter until after the RouteSyncer is created and assigned to LinuxRouter.
	linuxRouter := &routes.LinuxRouter{
		NodeIPA:       nrc.krNode,
		Tunneler:      nrc.tunneler,
		OverlayConfig: nrc.overlayConfig,
	}
	...
	linuxRouter.RouteSyncer = nrc.routeSyncer

	// Finally, now that the RouteSyncer is created and assigned to LinuxRouter, we can assign nrc.routerInjector
	nrc.routerInjector = linuxRouter

	// Kubernetes handler setup
