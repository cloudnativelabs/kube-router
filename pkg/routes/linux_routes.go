...
func (lr *LinuxRouter) InjectRoute(subnet *net.IPNet, gw net.IP) (bool, error) {
	...
	tunnelName := lr.Tunneler.GenerateTunnelName(gw.String())
	...
	var sameSubnet bool
	if gw.To4() != nil {
		sameSubnet = checkNHSameSubnet(gw, lr.NodeIPA.GetNodeIPv4Addrs())
	} else if gw.To16() != nil {
		sameSubnet = checkNHSameSubnet(gw, lr.NodeIPA.GetNodeIPv6Addrs())
	}
	...
	case link != nil:
		...
		if subnet.IP.To4() != nil {
			bestIPForFamily = lr.NodeIPA.FindBestIPv4NodeAddress()
		} else {
